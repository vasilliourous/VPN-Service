...
> REMOVED.** This document describes `legacy/wails-client/` — the retired Go +
> Wails + sing-box client. **The shipping client is `client/`** (the Tauri fork of
> Clash Verge Rev, tunnelling through mihomo), which has its own docs in
> `client/docs/`. The content was not otherwise reviewed after the 2026-09-23
> restructure. Kept because it documents the contract the fork must reproduce —
> see `legacy/wails-client/ARCHIVED.md`.
>
> Where this document names a root `VERSION` file, `.github/workflows/build.yml`,
> `.syso` resources, or the version-bump tooling, that machinery is **deleted**
> (see `docs/STILL-OPEN.md`). The **server** half of this document remains
> current.


> **Historical framing (do not read as current).** The paragraph below says "the
> current client architecture. The GUI layer is Wails + Vue 3" — that was true
> when written and is not now. The shipping GUI is the Tauri + React fork in
> `client/`.
...
(`rsrc_windows_amd64.syso` / `rsrc_windows_arm64.syso` — **these committed
resources have been deleted** along with the archived-client release tooling;
this describes how the retired build produced them, via
`go generate -tags windows` / `github.com/tc-hib/go-winres`). Windows therefore
