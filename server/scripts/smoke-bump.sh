...
#
# Usage: server/scripts/smoke-bump.sh [path-to-bump-version.sh]
#
# Exit codes: 0 all assertions held, 1 an assertion failed.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
BUMP="${1:-${REPO_ROOT}/server/scripts/bump-version.sh}"
...
SYSO_SRC=""
SYSO_SEED=""
if [ -f "${REPO_ROOT}/legacy/wails-client/rsrc_windows_amd64.syso" ] && command -v python3 >/dev/null 2>&1; then
    mkdir -p "$SCRATCH/seed"
    cp "${REPO_ROOT}/legacy/wails-client/rsrc_windows_amd64.syso" "$SCRATCH/seed/rsrc_windows_amd64.syso"
    cp "${REPO_ROOT}/legacy/wails-client/rsrc_windows_arm64.syso" "$SCRATCH/seed/rsrc_windows_arm64.syso"
...
# accounted files, and enough of legacy/wails-client for the drift scan to have something
...
setup_scratch() {
    rm -rf "${SCRATCH:?}/repo"
    mkdir -p "$SCRATCH/repo/legacy/wails-client/internal/buildinfo" \
             "$SCRATCH/repo/legacy/wails-client/frontend" \
             "$SCRATCH/repo/server/scripts"
    cd "$SCRATCH/repo"
    git init -q .
    git config user.email smoke@example.invalid
    git config user.name smoke

    echo -n "2.2.0" > VERSION
    cat > legacy/wails-client/main.go <<'EOF'
package main
...
EOF
    cat > legacy/wails-client/internal/buildinfo/buildinfo.go <<'EOF'
package buildinfo

const fallbackVersion = "2.2.0"
EOF
    cat > legacy/wails-client/wails.json <<'EOF'
{
  "name": "locus",
  "version": "2.2.0"
}
EOF
    cat > legacy/wails-client/frontend/package.json <<'EOF'
{
  "name": "locus-frontend",
  "version": "2.2.0",
...
EOF
    cat > legacy/wails-client/frontend/package-lock.json <<'EOF'
{
  "name": "locus-frontend",
  "version": "2.2.0",
  "lockfileVersion": 3,
...
EOF
    ...
    mkdir -p docs
    echo "shipped in 2.2.0" > docs/FIXES.md

    # Real scripts, so we exercise the code we are shipping rather than a copy.
    cp "$BUMP" server/scripts/bump-version.sh
    chmod +x server/scripts/bump-version.sh
    cp "$STAMP" server/scripts/stamp-syso.py
    chmod +x server/scripts/stamp-syso.py
    ...
    if [ -n "$SYSO_SEED" ]; then
        cp "$SYSO_SEED" legacy/wails-client/rsrc_windows_amd64.syso
        cp "$SYSO_SRC"  legacy/wails-client/rsrc_windows_arm64.syso
    fi
...
server/scripts/bump-version.sh patch --dry-run >"$SCRATCH/dry.log" 2>&1 || true
...
server/scripts/bump-version.sh patch --no-verify >"$SCRATCH/bump.log" 2>&1 || {
    echo "  (bump failed; log follows)"; sed 's/^/    /' "$SCRATCH/bump.log"; }
check "VERSION now reads 2.2.1" [ "$(cat VERSION)" = "2.2.1" ]
check "main.go runtime literal bumped" grep -q 'var version = "2.2.1"' legacy/wails-client/main.go
check "go:generate directive bumped" grep -q -- '--product-version 2.2.1 --file-version 2.2.1' legacy/wails-client/main.go
check "buildinfo fallback bumped" grep -q 'fallbackVersion = "2.2.1"' legacy/wails-client/internal/buildinfo/buildinfo.go
check "wails.json bumped" grep -q '"version": "2.2.1"' legacy/wails-client/wails.json
check "package.json bumped" grep -q '"version": "2.2.1"' legacy/wails-client/frontend/package.json
check "package-lock root bumped" grep -q '"version": "2.2.1"' legacy/wails-client/frontend/package-lock.json

# ── Case 3: no stale copy may survive in the client tree ───────────────────
echo "case 3 — no stale copy survives"
STALE="$(grep -rl --exclude-dir=.git --exclude-dir=docs -e '2\.2\.0' legacy/wails-client 2>/dev/null || true)"
check "old version is gone from legacy/wails-client" [ -z "$STALE" ]
check "docs history is left alone"   grep -q "2.2.0" docs/FIXES.md

# ── Case 4: the stray-copy detector must actually fire ─────────────────────
echo "case 4 — a seventh copy is detected, not tolerated"
setup_scratch
cd "$SCRATCH/repo"
mkdir -p legacy/wails-client/internal/newpkg
cat > legacy/wails-client/internal/newpkg/const.go <<'EOF'
package newpkg
...
if server/scripts/bump-version.sh patch --no-verify >"$SCRATCH/stray.log" 2>&1; then
...
setup_scratch
cd "$SCRATCH/repo"
if server/scripts/bump-version.sh 2.2.0 --no-verify >"$SCRATCH/same.log" 2>&1; then
    check "refuses a version equal to current" false
else
    check "refuses a version equal to current" true
fi
if server/scripts/bump-version.sh banana --no-verify >"$SCRATCH/banana.log" 2>&1; then
...
setup_scratch
cd "$SCRATCH/repo"
echo "// local edit" >> legacy/wails-client/main.go
if server/scripts/bump-version.sh patch --no-verify >"$SCRATCH/dirty.log" 2>&1; then
    check "refuses a dirty tree" false
else
    check "refuses a dirty tree" true
    check "the local edit is untouched" grep -q "// local edit" legacy/wails-client/main.go
    check "VERSION untouched" [ "$(cat VERSION)" = "2.2.0" ]
fi
...
for spec in "major:3.0.0" "minor:2.3.0" "patch:2.2.1"; do
    ...
    setup_scratch
    cd "$SCRATCH/repo"
    server/scripts/bump-version.sh "$word" --no-verify >/dev/null 2>&1 || true
    got="$(cat VERSION)"
...
if [ -z "$SYSO_SEED" ] || ! command -v python3 >/dev/null 2>&1; then
    ...
    check "seed artifacts read 2.2.0 before the bump" \
        python3 server/scripts/stamp-syso.py 2.2.0 --check --quiet

    # GO_BIN points at nothing: the point is that no toolchain is needed.
    GO_BIN=definitely-not-a-real-go \
        server/scripts/bump-version.sh patch --no-verify >"$SCRATCH/c7.log" 2>&1 || true

    check "VERSION advanced to 2.2.1" [ "$(cat VERSION)" = "2.2.1" ]
    check "artifacts were stamped to 2.2.1 (--no-verify did not skip them)" \
        python3 server/scripts/stamp-syso.py 2.2.1 --check --quiet
...
raw = open("legacy/wails-client/rsrc_windows_amd64.syso", "rb").read()
...
PY
    check "ProductName is still Locus, not the legacy brand" \
        bash -c "python3 server/scripts/stamp-syso.py 2.2.1 --check --quiet"
fi
...
if [ -z "$SYSO_SEED" ] || ! command -v python3 >/dev/null 2>&1; then
    ...
    setup_scratch
    cd "$SCRATCH/repo"
    if python3 server/scripts/stamp-syso.py 2.10.0 --client-dir legacy/wails-client \
    ...
    setup_scratch
    cd "$SCRATCH/repo"
    printf '\000\001\002\003\377\376' > legacy/wails-client/rsrc_windows_amd64.syso
    if python3 server/scripts/stamp-syso.py 2.2.1 --client-dir legacy/wails-client \
...
if [ -z "$SYSO_SEED" ] || ! command -v python3 >/dev/null 2>&1; then
    ...
    setup_scratch
    cd "$SCRATCH/repo"
    before="$(cksum legacy/wails-client/rsrc_windows_amd64.syso)"
    if python3 server/scripts/stamp-syso.py 2.2.5 --check --quiet >/dev/null 2>&1; then
        check "--check reports a mismatched artifact as stale" false
    else
        check "--check reports a mismatched artifact as stale" true
    fi
    check "--check wrote nothing" [ "$before" = "$(cksum legacy/wails-client/rsrc_windows_amd64.syso)" ]
